...
DEFAULT_REQUEST_TIMEOUT_SECONDS = int(os.environ.get("SMART_ATS_DEMO_REQUEST_TIMEOUT_SECONDS", "600"))
SEED_PREVIEW_IMAGE_DATA_URL = (
            ...
            "pagePreviews": [
                {
                    ...
                    "width": 320,
                    "height": 452,
                    "imageDataUrl": SEED_PREVIEW_IMAGE_DATA_URL,
...
def wait_for_job_recommendations(
    admin_client: JsonApiClient,
    job_mapping: dict[str, str],
    expected_min_count: int,
    timeout_seconds: int,
) -> None:
    if expected_min_count <= 0 or not job_mapping:
        return
    ...
    while time.time() < deadline:
        ...
        for dataset_job_id, actual_job_id in job_mapping.items():
            recommendations = admin_client.get(f"/api/jobs/{actual_job_id}/recommendations")
            current_counts[dataset_job_id] = len(recommendations)
            if current_counts[dataset_job_id] < expected_min_count:
                ready = False

        if ready:
            print(
                "[recommendations] ready: "
                + ", ".join(f"{job_id}={count}" for job_id, count in sorted(current_counts.items()))
            )
            return
    ...
    raise TimeoutError(
        "Recommendations did not reach the expected count within "
        f"{timeout_seconds} seconds: {last_counts}"
    )


def resolve_demo_hr_registration(admin_client: JsonApiClient) -> tuple[str, str]:
    organizations = admin_client.get("/api/admin/organizations")
    target = next(
        (
            organization
            for organization in organizations
            if organization.get("enabled") and organization.get("systemDefault")
        ),
        None,
    )
    if target is None:
        target = next((organization for organization in organizations if organization.get("enabled")), None)
    if target is None:
        raise RuntimeError("No enabled organization is available for demo HR registration")

    token_response = admin_client.post(f"/api/admin/organizations/{target['id']}/regenerate-token")
    return target["id"], token_response["generatedToken"]
...
def ensure_candidates(
    ...
    for sample in manifest:
        ...
        candidate_client, created = ensure_user(
            base_url,
            username,
            email,
            DEFAULT_PASSWORD,
        ...
        final_status = wait_for_resume(admin_client, existing_resume["id"], wait_timeout_seconds)
...
def evaluate_jobs(admin_client: JsonApiClient, job_mapping: dict[str, str], summary: Counter[str]) -> None:
    total_jobs = len(job_mapping)
    for index, actual_job_id in enumerate(job_mapping.values(), start=1):
        print(f"[evaluate] job {index}/{total_jobs} -> {actual_job_id}")
...
def main() -> None:
    ...
    admin_client = login(args.base_url, admin_username, admin_password, timeout=args.request_timeout_seconds)
    hr_organization_id, hr_organization_token = resolve_demo_hr_registration(admin_client)
    _, hr_created = ensure_user(
        args.base_url,
        "demo_hr",
        "demo_hr@smartats.local",
        DEFAULT_PASSWORD,
        "HR",
        organization_id=hr_organization_id,
        organization_token=hr_organization_token,
        timeout=args.request_timeout_seconds,
    )

    summary: Counter[str] = Counter()
    summary["hr_users_created" if hr_created else "hr_users_reused"] += 1
    summary["organization_tokens_regenerated"] += 1
    ...
    parsed_resume_count = sum(1 for candidate in candidates if candidate["resumeStatus"] == "PARSED")
    wait_for_job_recommendations(admin_client, job_mapping, parsed_resume_count, args.timeout_seconds)
    summary["recommendation_refresh_wait_completed"] += 1

    if args.force_evaluate and not args.skip_evaluate:
